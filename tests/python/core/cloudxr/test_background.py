...
class TestSpawn:
    ...
    def test_the_log_socket_is_not_inherited(self, tmp_path, monkeypatch):
        ...
        """
        monkeypatch.setenv("ISAACTELEOP_LOG_SOCKET", "/run/user/0/it/leader.sock")
        with patch("isaacteleop.cloudxr.background.subprocess.Popen") as popen:
            popen.return_value = MagicMock(pid=4242)
            background.spawn([], str(tmp_path / "run"), tmp_path / "logs")

        env = popen.call_args.kwargs["env"]
        assert "ISAACTELEOP_LOG_SOCKET" not in env
