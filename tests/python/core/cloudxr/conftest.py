...
* Synthetic package ``cloudxr_py_test_ns``: ``from cloudxr_py_test_ns.oob_teleop_env import …``
  ...
  ``isaacteleop`` root so that the ones reaching ``..`` load too.
"""
...
# ``wss`` reaches its own package's parent (``from .. import logging_config``), which a
# one-level synthetic package has nothing to resolve. This root stands in for
# ``isaacteleop`` and takes its ``__path__`` from the real source tree, so a sibling
...
# tests patch by.
_TEST_ROOT_PKG = "isaacteleop_py_test_ns"
