...
``install()`` runs from ``import isaacteleop``, so whatever it raises is raised
...
import isaacteleop  # noqa: F401 -- the import under test
log = logging.getLogger("isaacteleop.test.install")
...
def test_an_address_no_one_answers_is_not_believed(tmp_path, kind):
    ...
    result, report = probe(tmp_path, log_dir, ISAACTELEOP_LOG_SOCKET=str(dead))
...
def test_a_junk_log_level_falls_back_to_info(tmp_path):
    log_dir = usable_log_dir(tmp_path)
    result, _ = probe(tmp_path, log_dir, ISAACTELEOP_LOG_LEVEL="verbose-please")
...
logging.getLogger("isaacteleop.test.install").warning("probe warning")
with open(sys.argv[1], "w", encoding="utf-8") as handle:
    json.dump(
