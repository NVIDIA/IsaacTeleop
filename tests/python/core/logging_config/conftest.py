...
per process, so a permutation of the five variables *is* a process; there is no
...
#: The whole external contract. A child gets exactly what a test gives it.
CONTRACT_ENV_VARS = (
    "ISAACTELEOP_LOG_DIR",
    "ISAACTELEOP_LOG_LEVEL",
    "ISAACTELEOP_LOG_SOCKET",
    "ISAACTELEOP_NATIVE_CAPTURE",
    "ISAACTELEOP_NATIVE_CAPTURE_FILE",
)
...
def clean_env(log_dir: Path | str, **overrides: str) -> dict[str, str]:
    ...
    env["ISAACTELEOP_LOG_DIR"] = str(log_dir)
...
def session_logs(log_dir: Path) -> list[Path]:
    """``<timestamp>.isaacteleop.<pid>.log`` -- the Python half's own files."""
    return _matching(log_dir, r"[\d-]+\.isaacteleop\.\d+\.log")


def cpp_logs(log_dir: Path) -> list[Path]:
    """``<timestamp>.isaacteleop.<pid>.cpp[-N].log`` -- what local_sinks() opens."""
    return _matching(log_dir, r"[\d-]+\.isaacteleop\.\d+\.cpp(-\d+)?\.log")


def capture_logs(log_dir: Path) -> list[Path]:
    """``<timestamp>.isaacteleop.<pid>.native.log`` -- raw fd 1/2 output."""
    return _matching(log_dir, r"[\d-]+\.isaacteleop\.\d+\.native\.log")
...
import isaacteleop  # noqa: F401 -- importing it is what installs everything
