...
with logging_config.capture_native_output() as path:
...
""",
    ...
    # Outside a scope isaacteleop does not touch the host's descriptors at all.
...
def test_concurrent_scopes_leave_the_descriptors_as_they_were(tmp_path):
    result, log_dir, report = run_child(
        tmp_path,
        """
log = logging.getLogger("isaacteleop.test.capture")
...
def test_a_warning_inside_a_scope_still_reaches_the_terminal(tmp_path):
    ...
    result, log_dir, _ = run_child(
        tmp_path,
        """
log = logging.getLogger("isaacteleop.test.capture.console")
with logging_config.capture_native_output():
...
def test_capture_off_leaves_the_descriptors_alone_but_still_publishes_the_file(
    ...
    # The switch governs the host's descriptors; a process isaacteleop launches
...
report["published"] = os.environ.get("ISAACTELEOP_NATIVE_CAPTURE_FILE")
report["fd"] = logging_config.native_capture_fd() is not None
""",
        ISAACTELEOP_NATIVE_CAPTURE="off",
    )
...
def test_a_capture_file_nothing_wrote_to_is_removed_at_exit(tmp_path):
    # Most processes that import isaacteleop emit no raw fd output at all, and
    # the file has to exist before the first byte can land in it.
    _, log_dir, report = run_child(
        tmp_path,
        """
report["path"] = str(logging_config.native_capture_path())
...
def test_a_child_given_the_capture_fd_writes_into_it(tmp_path):
    result, log_dir, report = run_child(
        tmp_path,
        """
fd = logging_config.native_capture_fd()
report["fd"] = fd
subprocess.run(
