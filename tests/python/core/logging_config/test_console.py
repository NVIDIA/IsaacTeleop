...
def restore_console():
    ...
    level = handler.level
    published = os.environ.get("ISAACTELEOP_LOG_LEVEL")
    yield
    ...
    if published is None:
        ...
        os.environ["ISAACTELEOP_LOG_LEVEL"] = published


def record(name="isaacteleop.plugins.manus", message="gloves %s", args=("ready",)):
...
class TestKeywordFilter:
    ...
    def test_logger_name_target_ignores_the_message(self):
        ...
        assert not keep.filter(record(name="isaacteleop.core.Session"))
        assert not keep.filter(
            record(name="isaacteleop.core.Session", message="manus", args=None)
        )
    ...
    def test_both_is_the_union(self):
        ...
        assert not keep.filter(
            record(name="isaacteleop.core.Session", message="x", args=None)
        )
...
class TestConsoleFilterApi:
    def test_set_and_clear(self):
        ...
        assert not handler.filter(record(name="isaacteleop.core.Session"))

        logging_config.set_console_filter(None)
        assert handler.filter(record(name="isaacteleop.core.Session"))
...
class TestConsoleLevel:
    def test_publishes_the_level_a_plugin_executable_will_read(self):
        # fork+exec'd plugins are out of reach of the in-process bridge and
        # read their own threshold from this variable.
        logging_config.set_console_level("debug")
        assert _console.ensure_handler().level == logging.DEBUG
        assert os.environ["ISAACTELEOP_LOG_LEVEL"] == "debug"

        logging_config.set_console_level(logging_config.TRACE)
        assert os.environ["ISAACTELEOP_LOG_LEVEL"] == "trace"

    def test_publishes_a_number_for_a_level_with_no_name(self):
        logging_config.set_console_level(23)
        assert os.environ["ISAACTELEOP_LOG_LEVEL"] == "23"
...
def render(stream, level=logging.INFO, name="isaacteleop.core.Probe") -> str:
    handler = logging.StreamHandler(stream)
    formatter = _console._LoggerNameColorFormatter(
        logging_config.LINE_FORMAT,
...
class TestColourFormatter:
    ...
    def test_a_logger_colour_applies_inside_the_line_and_the_level_resumes(self):
        logging_config.set_logger_colors({"isaacteleop.core.Probe": "\033[36m"})
        line = render(_Tty(), logging.WARNING)
        assert f"[\033[36misaacteleop.core.Probe{_YELLOW}]" in line
    ...
    def test_a_logger_colour_on_an_uncoloured_level_resets_after_the_name(self):
        logging_config.set_logger_colors({"isaacteleop.core.Probe": "\033[36m"})
        assert f"[\033[36misaacteleop.core.Probe{_RESET}]" in render(_Tty())
    ...
    def test_the_record_is_handed_back_unmodified(self):
        ...
        entry = logging.LogRecord(
            "isaacteleop.core.Probe", logging.ERROR, __file__, 1, "msg", None, None
        )
        formatter.format(entry)
        assert entry.name == "isaacteleop.core.Probe"
...
class TestLoggerColours:
    ...
    def test_accepts_sgr(self, colour):
        logging_config.set_logger_colors({"isaacteleop.core.Probe": colour})
        assert _console._logger_colors["isaacteleop.core.Probe"] == colour
    ...
    def test_none_drops_a_colour(self):
        logging_config.set_logger_colors({"isaacteleop.core.Probe": "\033[36m"})
        logging_config.set_logger_colors({"isaacteleop.core.Probe": None})
        assert "isaacteleop.core.Probe" not in _console._logger_colors
