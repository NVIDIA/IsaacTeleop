...
class TestLevelVocabulary:
    ...
    def test_env_console_level_never_raises(self, monkeypatch, raw, expected):
        monkeypatch.setenv("ISAACTELEOP_LOG_LEVEL", raw)
...
class TestLineFormat:
    ...
    def _render(self, level: int, message: str, args) -> str:
        record = logging.LogRecord(
            "isaacteleop.core.Probe", level, __file__, 1, message, args, None
        )
    ...
    def test_renders_the_documented_fields(self):
        ...
        assert match["name"] == "isaacteleop.core.Probe"
    ...
    def test_renders_trace(self):
        rendered = LINE_RE.fullmatch(self._render(logging_config.TRACE, "x", None))
...
class TestLogDir:
    ...
    def test_defaults_to_a_per_uid_directory(self, monkeypatch):
        # sink_config.cpp builds this same string; a shared /tmp/isaacteleop
        # would belong to whichever user created it first.
        monkeypatch.delenv("ISAACTELEOP_LOG_DIR", raising=False)
        assert _core.log_dir() == Path(f"/tmp/isaacteleop-{os.getuid()}/logs")
    ...
    def test_republishes_an_absolute_path(self, monkeypatch, tmp_path):
        ...
        assert os.environ["ISAACTELEOP_LOG_DIR"] == str(resolved)
