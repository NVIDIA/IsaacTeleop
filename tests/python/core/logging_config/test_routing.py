...
log = logging.getLogger("isaacteleop.test.routing")
log.debug("python debug")
log.info("python info")
with open(sys.argv[1], "w", encoding="utf-8") as handle:
    json.dump(
        {
            "socket": os.environ.get("ISAACTELEOP_LOG_SOCKET"),
            "capture": os.environ.get("ISAACTELEOP_NATIVE_CAPTURE_FILE"),
            "pid": os.getpid(),
        },
        handle,
    )
...
class TestStandaloneCpp:
    ...
    def test_writes_a_line_in_the_python_format_to_its_own_file(self, tmp_path):
        name = "isaacteleop.plugins.probe.Standalone"
        result = run_emitter(
    ...
    def test_the_console_threshold_comes_from_the_environment(self, tmp_path):
        name = "isaacteleop.plugins.probe.Threshold"
        result = run_emitter(
            ["emit", name, "info", "below the bar", "warning", "above the bar"],
            clean_env(tmp_path, ISAACTELEOP_LOG_LEVEL="warning"),
        )
        assert result.returncode == 0
    ...
    def test_a_dead_address_sends_it_back_to_its_own_sinks(self, tmp_path):
        result = run_emitter(
            ["emit", "isaacteleop.plugins.probe.Dead", "info", "fell back"],
            clean_env(tmp_path, ISAACTELEOP_LOG_SOCKET=str(tmp_path / "nothing.sock")),
        )
        assert result.returncode == 0
    ...
    def test_the_two_halves_never_want_the_same_file_name(self, tmp_path):
        ...
        run_emitter(
            ["emit", "isaacteleop.plugins.probe.Shared", "info", "cpp line"],
            clean_env(shared),
        )
...
def test_a_standalone_cpp_process_forwards_into_the_leaders_file(leader, tmp_path):
    name = "isaacteleop.plugins.probe.Forwarded"
    child_logs = tmp_path / "child-logs"
    child_logs.mkdir()
    result = run_emitter(
        ["emit", name, "warning", "shipped to the leader"],
        clean_env(child_logs, ISAACTELEOP_LOG_SOCKET=leader.socket),
    )
    assert result.returncode == 0
    ...
    # Only the leader touches disk or a terminal for isaacteleop records.
    assert cpp_logs(child_logs) == []
    assert result.stderr == ""


def test_an_in_process_cpp_logger_reaches_the_python_handlers(tmp_path):
...
import isaacteleop  # noqa: F401
from isaacteleop.plugin_manager import PluginManager
...
def logged():
    text = ""
    for name in glob.glob(os.path.join(os.environ["ISAACTELEOP_LOG_DIR"], "*.log")):
...
while time.monotonic() < deadline and "Error parsing metadata" not in logged():
    time.sleep(0.05)
with open(sys.argv[1], "w", encoding="utf-8") as handle:
    json.dump({"socket": os.environ.get("ISAACTELEOP_LOG_SOCKET")}, handle)
""",
        clean_env(log_dir),
        str(report_path),
    ...
    assert match is not None
    assert match["name"] == "isaacteleop.core.PluginManager"
...
class TestProcessShape:
    def test_a_forwarding_child_keeps_no_console_and_no_file(self, leader, tmp_path):
        ...
        result, report = run_probe(
            tmp_path, child_logs, ISAACTELEOP_LOG_SOCKET=leader.socket
        )
...
if override:
    os.environ["ISAACTELEOP_NATIVE_CAPTURE_FILE"] = override

manager = PluginManager([search])
plugin = manager.start("capture_probe", "probe-root")
watched = override or str(logging_config.native_capture_path())
