...
def test_an_explicit_silence_request_is_honoured(argv, env):
    """Each moves an ignore filter ahead of PEP 565's
    ``default::DeprecationWarning:__main__``, which is what the guard probes for."""
...
def test_a_pytest_ini_filter_is_honoured(tmp_path):
    ...
    invisible to ``sys.warnoptions``, and pytest applies it around collection, where
    a downstream's module-level ``import isaacteleop`` runs. ``-s``: pytest would
    otherwise capture the very stderr under test."""
...
def test_the_printed_notice_names_the_importing_line(tmp_path):
    """The print branch is the one every non-``__main__`` import takes, so "which of
    my modules imported this?" has to be answerable from it."""
