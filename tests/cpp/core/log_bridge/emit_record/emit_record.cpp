...
//
// tests/python/core/logging_config/test_logging_config.py drives this binary for
// both of those states -- the local one in
// test_cpp_logger_without_socket_uses_own_console_and_file, the forwarding one in
// test_cpp_logger_reaches_the_python_receiver, which is the only place a real C++
// sender and a real Python receiver are ever checked against each other. CMake
// hands the path over in ISAACTELEOP_LOG_EMITTER; see this directory's
// CMakeLists.txt and tests/python/core/logging_config/CMakeLists.txt.
//
