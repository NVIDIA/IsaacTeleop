...
# Both spellings: `isaacteleop` is only a runtime alias, so a doc that still
# imports it is as copy-pasteable-broken as one naming a module that never
# existed. The (?!\w) is what keeps `import isaacteleop_examples.robot_viz` --
# a separate, deliberately unrenamed namespace package -- from capturing
# `isaacteleop` and resolving nowhere.
