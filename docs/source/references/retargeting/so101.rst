...
* :class:`~isaacteleop.retargeters.AnalogGripperRetargeter` -- proportional (analog) jaw
  closedness from the controller trigger. The closedness is unitless, so this one is not
  SO-101-specific.
...
``AnalogGripperRetargeter`` maps the analog trigger to a jaw **closedness** ``c`` in ``[0, 1]``
...
.. code-block:: python
   ...
   def build_so101_stack_pipeline(base_T_gripper_home: np.ndarray):
       ...
       gripper = AnalogGripperRetargeter(name="gripper", input_device=ControllersSource.RIGHT)
