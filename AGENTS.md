...
- **Six environment variables are the whole external contract.** The first
  ...
  the two stop agreeing: `ISAACTELEOP_LOGGING` (see the next rule),
  `ISAACTELEOP_LOG_DIR` (where log files land),
...
- **`ISAACTELEOP_LOGGING=off` switches this whole system off**; unset or any
