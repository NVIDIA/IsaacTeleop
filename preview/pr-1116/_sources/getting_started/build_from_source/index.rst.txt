...
.. code-block:: bash
   ...
   pip install "isaaccapture[retargeters,cloudxr,ui]" isaacteleop --find-links=./install/wheels/ --no-index --force-reinstall

.. code-block:: bash

   # Pass --reinstall to replace an existing install.
   uv pip install "isaaccapture[retargeters,cloudxr,ui]" isaacteleop --find-links=./install/wheels/ --reinstall

The build produces a second wheel, ``isaacteleop`` — the compatibility alias,
removed in 1.9. It is a separate distribution, so naming ``isaaccapture`` alone
leaves ``import isaacteleop`` a ``ModuleNotFoundError``; drop it from the command
once nothing you run imports the old name. Name it bare: its only dependency is
the ``isaaccapture`` already in ``./install/wheels/``, so it adds nothing for
``--no-index`` to resolve, and extras on the alias would route the same
requirement through a second distribution for no gain. See
...
.. admonition:: ``import isaacteleop`` under this path
   ...
   ``pip install -e .`` serves the alias. An editable install resolves imports out
   of the source tree, and the shim's source (:code-dir:`src/compat/isaacteleop`)
   is redirected alongside ``isaaccapture``'s, so the old name keeps working while
   you migrate.

   ``pip install .`` does **not**, by design: that command builds the real
   ``isaaccapture`` distribution, which ships exactly one first-party package. The
   alias belongs to the separate ``isaacteleop`` distribution
   (:doc:`/references/migration`). To have it beside a non-editable install, add
   the compatibility project the build stages:
