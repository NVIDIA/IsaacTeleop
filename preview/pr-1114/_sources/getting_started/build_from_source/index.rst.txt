...
leaves ``import isaacteleop`` a ``ModuleNotFoundError``; drop it once nothing you
run imports the old name. See :doc:`/references/migration`.
...
.. admonition:: ``import isaacteleop`` under this path
   ...
   is redirected alongside ``isaaccapture``'s. ``pip install .`` does not -- the
   alias is the separate ``isaacteleop`` distribution
   (:doc:`/references/migration`). To have it beside a non-editable install:

   .. code-block:: bash

      pip install ./build-wheel/cpython-311/python_package_compat/Release

   Substitute your interpreter's cache tag for ``cpython-311``.
