#!/usr/bin/env python3
"""
Replay a saved Sharpa HA4 (or other URDF) hand joint trajectory in Isaac Sim.
...
Usage:
  ./isaaclab.sh -p scripts/demos/sharpahand_replay_traj.py \
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Replay Sharpa (or custom URDF) hand trajectory in Isaac Sim.")
    parser.add_argument(
        "--traj_npz",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Local URDF path for the LEFT hand (required for --hand left/both).",
    )
    # Backward-compatible alias (matches other scripts that use kebab-case flags).
    parser.add_argument(
        "--left-hand-urdf",
        dest="left_hand_urdf",
        type=str,
        default=None,
        help="Alias for --left_hand_urdf.",
    )
    parser.add_argument(
        "--right_hand_urdf",
        type=str,
        default=None,
        help="Local URDF path for the RIGHT hand (required for --hand right/both).",
    )
    # Backward-compatible alias (matches other scripts that use kebab-case flags).
    parser.add_argument(
        "--right-hand-urdf",
        dest="right_hand_urdf",
        type=str,
        default=None,
        help="Alias for --right_hand_urdf.",
    )
    parser.add_argument(
        "--mesh_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional LEFT-hand mesh directory (overrides --mesh_dir for the left hand).",
    )
    parser.add_argument(
        "--mesh_dir_right",
        type=str,
        default=None,
        help="Optional RIGHT-hand mesh directory (overrides --mesh_dir for the right hand).",
    )
    ...
    # NOTE: For Sharpa/custom URDF replay we don't rely on Nucleus assets.
    # We keep the import here because other parts of this script may use IsaacLab utilities.
    ...
    def _load_object_pose_npz(object_pose_path: str) -> tuple[np.ndarray, np.ndarray]:
        ...
        if "R_obj_cam" in obj.files and "t_obj_cam" in obj.files:
            ...
            if T.ndim != 3 or T.shape[-2:] != (4, 4):
                raise ValueError(f"T_obj_cam must have shape (T,4,4); got {T.shape}")
            R = T[:, :3, :3]
            t = T[:, :3, 3]
        else:
            raise KeyError(
    ...
    def _make_hand(prim_path: str, *, hand: str, pos_xyz: tuple[float, float, float]) -> Articulation:
        ...
        # Use user-provided URDFs (required for Sharpa).
        if hand == "right":
            if args.right_hand_urdf is None:
                raise ValueError("--right_hand_urdf is required for --hand right/both.")
            hand_urdf_local = str(Path(args.right_hand_urdf).expanduser().resolve())
        else:
            if args.left_hand_urdf is None:
                raise ValueError("--left_hand_urdf is required for --hand left/both.")
            hand_urdf_local = str(Path(args.left_hand_urdf).expanduser().resolve())

        if not Path(hand_urdf_local).is_file():
            raise FileNotFoundError(f"[urdf] {hand}: not a file: {hand_urdf_local}")
        print(f"[urdf] {hand}: {hand_urdf_local}")

        # Mesh staging for visibility.
        # - If URDF references relative paths like `meshes/*.STL`, Isaac Sim resolves those relative to the URDF folder.
        # - Many third-party URDFs reference `package://<pkg>/meshes/...`; Isaac Sim won't resolve ROS packages.
        #   We rewrite those to `meshes/...` and then symlink `meshes/` to a provided directory.
        mesh_dir_arg = None
        if hand == "right":
            mesh_dir_arg = args.mesh_dir_right if args.mesh_dir_right is not None else args.mesh_dir
        else:
            mesh_dir_arg = args.mesh_dir_left if args.mesh_dir_left is not None else args.mesh_dir

        # If mesh_dir wasn't provided, try to infer it as "<urdf_dir>/meshes".
        if mesh_dir_arg is None:
            inferred = Path(hand_urdf_local).resolve().parent / "meshes"
            if inferred.is_dir():
                mesh_dir_arg = str(inferred)

        if mesh_dir_arg is not None:
            mesh_dir = Path(mesh_dir_arg).expanduser().resolve()
            if not mesh_dir.is_dir():
                which = "--mesh_dir_right" if hand == "right" else "--mesh_dir_left"
                if (hand == "right" and args.mesh_dir_right is None) or (hand == "left" and args.mesh_dir_left is None):
                    which = "--mesh_dir"
                raise FileNotFoundError(f"{which} does not exist or is not a directory: {mesh_dir}")

            src_urdf = Path(hand_urdf_local).resolve()
            stage_dir = Path(tempfile.mkdtemp(prefix=f"isaaclab_sharpahand_replay_{hand}_"))
            staged_urdf = stage_dir / src_urdf.name
            shutil.copyfile(src_urdf, staged_urdf)

            # Patch common package:// mesh references to local-relative `meshes/`.
            try:
                txt = staged_urdf.read_text(encoding="utf-8")
                txt2 = re.sub(r"package://[^/]+/meshes/", "meshes/", txt)
                if txt2 != txt:
                    staged_urdf.write_text(txt2, encoding="utf-8")
            except Exception as e:
                raise RuntimeError(f"Failed to patch URDF mesh paths for {hand}: {staged_urdf} ({e})") from e
