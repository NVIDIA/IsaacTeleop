...
# Root of the mano_hand_retargeter package (examples/mano_hand_retargeter)
...
def resolve_cfg_paths(cfg):
    ...
    for name, rel_path in paths.items():
        if rel_path is None:
            # Treat None as "use ROOT_DIR"
            paths[name] = ROOT_DIR
            continue
        if not isinstance(rel_path, str):
            rel_path = str(rel_path)
        if rel_path.strip().lower() in {"none", "null", "~", ""}:
            paths[name] = ROOT_DIR
            continue
        if rel_path.startswith("/"):  # absolute path
            continue
        paths[name] = os.path.normpath(os.path.join(ROOT_DIR, rel_path))
