...
// BT.709 limited-range coefficients (ITU-R BT.709). Y is rescaled
// from [16, 235] -> [0, 255] via 1.16438 = 255/219; UV chroma scale
// is implicit in the 1.79274 / 0.21325 / 0.53291 / 2.11240 factors
// below (255/224 incorporated).
__global__ void nv12_to_rgba_limited_bt709_kernel(const uint8_t* __restrict__ y_plane,
                                                  const uint8_t* __restrict__ uv_plane,
                                                  int y_pitch,
                                                  uint8_t* __restrict__ dst,
                                                  int dst_pitch,
                                                  ...
                                                  int height)
{
    ...
    const float Y = static_cast<float>(y_plane[y * y_pitch + x]) - 16.0f;
    const int uv_x = x & ~1;
    const int uv_y = y >> 1;
    const int uv_offset = uv_y * y_pitch + uv_x;
    ...
    const float Yp = 1.16438f * Y;
    const float R = Yp + 1.79274f * Cr;
    const float G = Yp - 0.21325f * Cb - 0.53291f * Cr;
    const float B = Yp + 2.11240f * Cb;
                                ...
                                cudaStream_t stream)
{
    const dim3 block(16, 16);
    const dim3 grid((width + block.x - 1) / block.x, (height + block.y - 1) / block.y);
    nv12_to_rgba_limited_bt709_kernel<<<grid, block, 0, stream>>>(
