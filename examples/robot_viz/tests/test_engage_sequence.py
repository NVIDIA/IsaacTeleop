...
What this covers that the unit tests cannot is the *plumbing* -- three leaves out of
``before_step``, the gate node judging them inside the graph, and the clutch reading the
gate's verdict on the same frame. Every value here is the shipped pipeline's.
"""
...
from isaacteleop.retargeters.engage_gate import (  # noqa: E402
    KEY_ROTATION,
    KEY_UNREFERENCED,
)
...
from isaacteleop.viz.robot import SceneTwin  # noqa: E402
from isaacteleop.viz.robot.quaternion import multiply  # noqa: E402
from isaacteleop_examples.robot_viz import follower, harness  # noqa: E402
...
class _Rig:
    ...
    def __init__(self) -> None:
        ...
        if missing:
            pytest.skip(
                f"SO-101 assets not fetched ({', '.join(missing)}); "
                f"run {app.FETCH_SCRIPT}"
            )
        self.twin = SceneTwin(app.DEFAULT_SCENE)
        self.arm = follower.Follower(self.twin)
        self.pipeline, self.clutch, self.gate = app._build_pipeline(
            app.pose_from_ghost_body(*self.arm.gripper_pose_mj())
        )
        self.preview = app._Preview(
            self.twin,
            harness.InterventionMonitor(self.twin),
            self.arm,
            self.clutch,
            self.gate,
        )
        ...
        self._home_pos = app.pose_from_ghost_body(*self.arm.gripper_pose_mj())[:3, 3]
    ...
    def _controllers(self, orientation, squeeze: float):
        ...
        if orientation is None:
            orientation = app.grip_quat_from_ghost_body(self.arm.gripper_pose_mj()[1])
...
def _pitched(quat_xyzw, deg: float) -> np.ndarray:
    ...
    Pitch and not yaw, and that is the gate's whole shape here: ``Follower.drive`` turns
...
def test_an_aligned_wrist_settles_green_and_latches(rig):
    ...
    assert rig.preview.phases.phase is follower.ClutchPhase.ENGAGED


def test_a_turned_wrist_blocks_the_latch(rig):
    """The conjunct the gate exists for: 40 deg off, a squeeze must not latch."""
    turned = _pitched(app.grip_quat_from_ghost_body(rig.arm.gripper_pose_mj()[1]), 40.0)
...
def test_a_release_re_arms_and_the_dwell_debounces_it(rig):
    ...
    assert rig.frame(squeeze=0.0) is follower.ClutchPhase.DISENGAGED
    assert not rig.clutch.is_engaged
