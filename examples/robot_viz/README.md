...
Both must land in one environment. Beware `uv pip install isaaccapture` without a path or `--find-links`: a published `isaaccapture` exists on PyPI and will resolve, with no robot twin in it.
