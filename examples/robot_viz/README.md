...
A robot twin rendered stereoscopically into an Isaac Teleop Televiz XR
...
Single process, **one** OpenXR session, two threads:

```
TeleopSession ──┬── DeviceIOSession ── trackers ── the retargeting graph
                │                                          │
                └── render thread ── SceneTwin ◀── publish(…)
                                        │
mjr_render ─blit─▶ flip + depth-invert ─glReadPixels─▶ PBO ═CUDA═▶ submit()
```

That is the thesis: `TeleopSession` owns both halves — it creates the OpenXR
session the trackers and the compositor share, and runs the twin's frame loop on
a thread of its own — and **MuJoCo's own renderer** reaches
`ProjectionLayer.submit()` by CUDA pointer with no copy through host memory.
Nothing else in this repository does that.

`app.py` holds no `mjModel` and no `mjData`. It addresses the scene by name and
publishes what moved; `twin.py` applies the lot on the render thread, runs one
`mj_forward` and draws. Four things move — a body's pose, the joint array, a
group's visibility, a material's colour — and one thing is measured, a frame's
offset from another, **once, at load**. That bound is what lets the backend be
linked against a MuJoCo the user's environment knows nothing about.

**The backend is a readback, not a renderer.** `mjr_render` draws into MuJoCo's
offscreen framebuffer; `src/viz/robot_twin/cpp/gl_readback.cpp` blits that into a sampleable pair,
runs one fullscreen pass, and reads the result into a pixel-pack buffer that
CUDA imports. Every step stays in video memory. The GL half of that module — `gl.*`,
...
**MuJoCo is an implementation detail of `isaacteleop`, not a dependency of it.**
The backend lives in `src/viz/robot_twin/`, ships with Televiz on Linux, and
ships **its own MuJoCo under a private name** — `libisaacteleop_mujoco.so`,
loaded by `dlopen`/`dlsym`, so the extension exports exactly one symbol,
`PyInit__robot_twin`, and has no undefined `mj*` for another `libmujoco` to
answer. So whatever `mujoco` the environment has, at whatever version or
none at all, is unrelated to the twin's; `src/viz/robot_twin_tests/` runs both in
one process and checks they cannot see each other.

The name-only API above is what makes that safe. Were an `mjModel*` allocated by
a `mujoco` wheel and dereferenced by field offset in our extension, two
libmujocos would make that version skew *undetectable* rather than merely
unsafe. Nothing outside `scene.py` holds a scene handle, so there is no second
copy's layout to agree with.

This example is therefore **pure Python** — no compiled extension, no ABI tag,
no `mujoco` pin.
...
| | |
|---|---|
| **Covered by tests** | [`ctest -L robot_viz`](#tests) — the frame conventions, the frustum, the clock, the ghost overlay and its jaw channel, the safety harness the ghost renders, the follower's head anchoring and rigid drag, and the whole clutch handoff driven at frame rate through the real pipeline, all pure CPU; all pure CPU. The renderer's own tests, including the GPU readback, moved to the renderer: `ctest -L viz_python -R robot_twin`. |
...
one. It cannot show a wrong `src/viz/robot_twin/cpp/frames.hpp`: those constants place it, and the
...
Two calibrations, and they are different in kind. `src/viz/robot_twin/cpp/frames.hpp` is a
...
**Nothing here is compiled.** What has to be built is `isaacteleop` itself; this
example is pure Python that imports it.
...
There is no flag for the twin: a Linux `BUILD_VIZ` build has it. The first
configure fetches MuJoCo and its six vendored dependencies from GitHub and
compiles them (~40 s on 12 cores). A Windows Televiz build omits the twin — its
headless OpenGL context is EGL.

That is the path `ctest` needs. To just *run* the thing, `uv pip install .` drives
the same CMake and skips the `build/` tree entirely:
...
Both must land in **one** environment. Beware `uv pip install isaacteleop` without a
path or `--find-links`: a published `isaacteleop` exists on PyPI and will resolve, with
no robot twin in it.

`-e` is the way to iterate: the example is pure Python, so there is no extension to go
stale behind an editable install.

Running additionally needs a GPU with EGL + CUDA, and a headset.

**On a multi-GPU host, pass the GPU.** The OpenGL context has to land on the same
card viz picked, and nothing makes that happen by default: `SceneTwin` takes a
`gl_device_index`, which indexes EGL devices and need not agree with CUDA's
...
### The in-tree CMake build

`ctest -L robot_viz` runs the example's tests against the staged
`build/python_package/<config>/`, so the example is only added to the build where
the twin is. The twin's own tests are `ctest -R robot_twin`.

**`tests/CMakeLists.txt` globs `test_*.py` at configure time**, so adding or
deleting a test file leaves the ctest entry list stale until you re-configure.
...
```bash
python -m isaacteleop.rig rigs/robot_viz.yaml
```

`{python}` in the rig expands to the interpreter you launch it with, so the
`isaacteleop` wheel has to be installed *there* — not in the build venv. Picking
up the wrong venv is silent, so check before you start:

```bash
python -c "import sys, isaacteleop; from isaacteleop.viz.robot import SceneTwin; print(sys.executable, isaacteleop.__file__)"
```

That import is the whole check: it fails on an `isaacteleop` built without the
twin, and says so. The app's startup log prints the `isaacteleop:` line for the
same reason. Against a runtime you
started yourself:
...
an OpenXR error before any of this example's code runs — **no `[robot_viz]`
...
re-measure the leak on a headset. `viz.robot.yaw_of_axis` takes the axis as a
**required** argument for exactly this reason; `viz.robot.yaw_of` keeps `-Z` and is
...
**Green means all three of these hold**, and
`isaacteleop.retargeters.EngageAlignmentGate` — a graph node, not app code —
returns every one that does not, which `app.py` logs on each transition:
...
The first and third are the node's; the second is this app's, supplied on
`EngageAlignmentGate.APP_PERMITTED_INPUT` and named `("limiter", "still catching
up")` so the operator's log says what actually blocked. The node emits straight
into `ENGAGE_PERMITTED_INPUT`, which is what removes the round trip through app
code — but the graph is a pull-based DAG and cannot read the clutch it feeds, so
three leaves go the other way each frame: the reference pose, the app's conjunct,
and the phase machine's `permits_engagement`. All three carry frame N-1's answer;
squeezing inside that 14 ms costs nothing, because a denied latch stays **owed**.
...
wherever the hand goes, including places no articulated SO-101 could reach. The
node says so where the conjunct used to be; do not add one back believing it
keeps the operator inside a workspace this preview does not have.
...
base and every frame on it derived from two constants measured at `Q_HOME`, and
the ghost is two mocap bodies.
...
`mj_forward` after every joint, body-pose *or* mocap write, before every
...
`SceneTwin.render` owns that call and is the only place it appears, which is what
the publish-and-apply split buys: a caller cannot forget it, because a caller
cannot write to the scene at all.
...
enable precondition, not a safety-rated stop. `EngageAlignmentGate` is what fills
it here; the alignment math is shared with any other consumer rather than living
in this example.
...
--reinstall-package isaacteleop-examples-robot-viz ./examples/robot_viz`,
...
```bash
examples/robot_viz/scripts/fetch-so-arm.sh          # from the repository root
uv pip install --reinstall-package isaacteleop-examples-robot-viz ./examples/robot_viz
```
...
```bash
ctest --test-dir build -L robot_viz --output-on-failure          # this example
ctest --test-dir build -R robot_twin --output-on-failure         # the backend it drives
```
...
| `test_scene_twin.py` | the whole `SceneTwin` API on inline scenes: that a published change reaches the scene only on the render thread's apply, that publishes merge rather than replace and the latest wins, that a mocap body moves through `mocap_*` and a fixed one through `body_pos` without the caller choosing, that `drawn_only` leaves the collision geoms out, and that the load-time frame offsets are measured in the parent's own frame and frozen by `home()` |
...
frame loop that sequences it, the OpenXR session the compositor and the trackers
share, whether the runtime accepts the depth layer, and **controllers on a shared
session** — none of it is executed by any test or on any machine here. Since the
render loop moved onto its own thread there is one more: **`xrSyncActions` now
runs concurrently with `xrWaitFrame` / `xrBeginFrame` / `xrEndFrame`.** That is
legal per the OpenXR spec and unverified against CloudXR; the measurement harness
and its pass criteria are in
`/code/work/20260820-robot-twin-library/s2-xrsync-threading/`. `test_readback.py`
