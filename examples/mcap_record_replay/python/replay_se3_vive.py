...
def capture_rate_hz(mcap_path: Path, default: float = 30.0) -> float:
    """Estimate the recording's per-tracker sample rate from one sample channel."""
    summary = _summary(mcap_path)
    ...
    if span_ns <= 0:
        return default
    # Count messages on a single per-sample "/se3_tracker" channel (not the
    # "_tracked" one) so the rate reflects one tracker's sample cadence.
    per_sample_ids = [
        cid
        for cid, ch in summary.channels.items()
        if ch.topic.endswith("/se3_tracker")
        and not ch.topic.endswith("/se3_tracker_tracked")
    ]
    if not per_sample_ids:
        return default
    counts = stats.channel_message_counts
    best = max((counts.get(cid, 0) for cid in per_sample_ids), default=0)
    if best <= 1:
        return default
