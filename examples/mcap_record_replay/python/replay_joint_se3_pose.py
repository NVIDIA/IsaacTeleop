...
Per frame it prints each tip's distance from the thumb tip, which is what makes a
thumb-to-fingertip pinch readable as text: the touched finger's distance collapses toward
...
def pinch_distances(data) -> str:
    """Thumb-to-fingertip distances in mm, or a reason there are none."""
    if data is None:
        return "-"
    ...
    for label, joint in FINGERS:
        ...
        parts.append(f"{label}={mm:5.1f}")
    return " ".join(parts)


def run_once(session, trackers, rate_hz: float) -> tuple[int, dict]:
    frames = 0
    ...
    while none_streak < 30:
        ...
        for cid, tracker in trackers.items():
            data = tracker.get_data(session).data
            if data is not None:
                any_data = True
                samples[cid] += 1
            parts.append(f"{cid}: {pinch_distances(data)}")
        none_streak = 0 if any_data else none_streak + 1
        if frames % 60 == 1:
            print(f"[replay-joints] frame={frames:5d}  " + "  ".join(parts))
        if tick:
            time.sleep(tick)
...
def main(argv: list[str]) -> int:
    ...
    while True:
        ...
        with ReplaySession.run(config) as session:
            frames, samples = run_once(session, trackers, rate)
        print(f"[replay-joints] done — {frames} frames")
        for cid in collections:
            print(f"[replay-joints]   {cid}: {samples[cid]} frames with data")
        if not args.loop:
            break
