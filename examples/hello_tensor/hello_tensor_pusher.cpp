...
using namespace plugins::hello_tensor;
...
{
    std::string plugin_root_id = "hello_tensor";
    ...
    std::cout << "Hello Tensor Plugin" << std::endl;
    std::cout << "Plugin Root ID: " << plugin_root_id << std::endl;

    std::unique_ptr<HelloTensorPlugin> plugin;
    try
    {
        plugin = std::make_unique<HelloTensorPlugin>(plugin_root_id);
    }
    ...
    while (!g_stop_requested.load(std::memory_order_relaxed) && !plugin->is_done())
    {
    ...
    if (plugin->is_done())
    {
        std::cout << "\nWorker thread finished." << std::endl;
    }
    else
    {
