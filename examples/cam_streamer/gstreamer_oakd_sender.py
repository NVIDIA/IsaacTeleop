...
def create_camera_pipeline(args):
    ...
    encode_output = cam.requestOutput(
        (args.width, args.height), type=dai.ImgFrame.Type.NV12, fps=args.fps
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="OAK-D camera with H.264 RTP streaming"
    )
    parser.add_argument("--width", type=int, default=1280, help="Frame width")
    parser.add_argument("--height", type=int, default=720, help="Frame height")
    parser.add_argument("--fps", type=int, default=50, help="Frame rate")
    parser.add_argument(
        "--bitrate", type=int, default=20_000_000, help="H.264 bitrate (bps)"
    )
    parser.add_argument(
        "--quality", type=int, default=80, help="H.264 quality (1-100, higher=better)"
    )
    parser.add_argument(
        "--stream-host", type=str, default="127.0.0.1", help="RTP destination host"
    )
    parser.add_argument(
        "--stream-port", type=int, default=5000, help="RTP destination port"
    )
    parser.add_argument(
        "--retry-interval", type=int, default=10, help="Camera retry interval (seconds)"
    )
    args = parser.parse_args()

    print(f"DepthAI version: {dai.__version__}")
    print(
        f"Configuration: {args.width}x{args.height} @ {args.fps}fps, {args.bitrate / 1_000_000:.1f}Mbps"
    )
