...
//
// WHY BOTH AN IMAGE AND A BUFFER: we render into an OPTIMAL-tiled VkImage
// (that is what a colour/depth attachment has to be) and then
// vkCmdCopyImageToBuffer into a tightly-packed VkBuffer whose VkDeviceMemory
// was allocated exportable. CUDA imports THAT and gets a plain linear device
// pointer via cudaExternalMemoryGetMappedBuffer, which is what
// __cuda_array_interface__ describes. Importing the tiled image directly
// yields a cudaArray_t, which viz::VizBuffer explicitly does not model.
//
// The Vulkan device is BORROWED from isaacteleop.viz.VizSession, never
// created here. viz already enables VK_KHR_external_memory +
// VK_KHR_external_memory_fd on both of its device-creation paths and enforces
// them at physical-device selection, so borrowing gets the export path with
// zero viz changes -- and it guarantees the CUDA device already matches the
// Vulkan physical device by UUID, because VkContext::init() did that match.
