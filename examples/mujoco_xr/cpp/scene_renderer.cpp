...
struct PushConstants
{
    ...
    float ncol0[4]; // normal-matrix columns
    float ncol1[4];
    float ncol2[4];
    float color[4];
};
    ...
    : dev_(dev), config_(config)
{
    ...
    if (config_.view_count == 0 || config_.view_count > 2)
    {
        throw std::invalid_argument("mujoco_xr: view_count must be 1 (mono) or 2 (stereo)");
    }
...
void SceneRenderer::create_pipeline()
{
    ...
    blend.blendEnable = VK_TRUE; // marker decor is translucent and drawn last
    blend.srcColorBlendFactor = VK_BLEND_FACTOR_SRC_ALPHA;
    blend.dstColorBlendFactor = VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;
    blend.colorBlendOp = VK_BLEND_OP_ADD;
    // THE ALPHA CHANNEL IS THE AR PASSTHROUGH MASK -- when the runtime reports a
    // non-opaque environment blend mode, which is what makes viz set
    // XR_COMPOSITION_LAYER_BLEND_TEXTURE_SOURCE_ALPHA_BIT
    // (src/viz/session/cpp/xr_backend.cpp:1210). On an opaque VR runtime that
    // flag is 0 and destination alpha is discarded; the blend below is correct
    // either way, so this is a statement about what the alpha MEANS, not a
    // precondition. Compositing it, not replacing it:
    // A = A_src + (1 - A_src) * A_dst. With dstAlpha = ZERO the destination
    // alpha would simply become A_src, so a translucent marker (alpha 0.65)
    // drawn over an already-opaque geom would drop that pixel's alpha to 0.65
    // and the compositor would blend 35 % passthrough THROUGH the robot. The
    // colour factors above are the ordinary "over" applied to UNPREMULTIPLIED
    // source colours; these two are the same operator on alpha.
    //
    // What lands in the framebuffer is therefore PREMULTIPLIED -- a 0.65 marker
    // over the alpha-0 clear leaves (0.65*C, 0.65) -- and that is what the
    // runtime is being told to expect, because viz never sets
    // XR_COMPOSITION_LAYER_UNPREMULTIPLIED_ALPHA_BIT on any layer, which under
    // the OpenXR spec means premultiplied. Note the comment at
    // src/viz/session/cpp/xr_backend.cpp:1202-1203 claims the opposite
    // ("Straight alpha (not premultiplied)") while the code beside it sets no
    // such bit. Believe the code -- same trap as the "reverse-Z" doc comments
    // called out in render_target.hpp.
...
int SceneRenderer::update_scene(const mjModel* model, mjData* data)
{
    ...
    base_ngeom_ = scene_.ngeom;
    return base_ngeom_;
}

void SceneRenderer::clear_markers()
{
    scene_.ngeom = base_ngeom_;
}

void SceneRenderer::add_marker(const std::array<double, 3>& pos_mj,
                               ...
                               const std::array<float, 4>& rgba)
{
    ...
    {
        throw std::runtime_error("mujoco_xr: mjvScene is full (ngeom == maxgeom); raise kMaxGeom");
    }
    mjvGeom* g = scene_.geoms + scene_.ngeom;
    const mjtNum size[3] = { half_extent[0], half_extent[1], half_extent[2] };
    mjtNum mat[9];
    mju_quat2Mat(mat, quat_mj_wxyz.data());
    const mjtNum pos[3] = { pos_mj[0], pos_mj[1], pos_mj[2] };
    const float rgba_f[4] = { rgba[0], rgba[1], rgba[2], rgba[3] };
    mjv_initGeom(g, mjGEOM_BOX, size, pos, mat, rgba_f);
    ++scene_.ngeom;
}
...
void SceneRenderer::render(const std::vector<float>& poses_xyz_qwxyz, const std::vector<float>& fovs_lrud)
{
    ...
    for (size_t v = 0; v < n; ++v)
    {
        ...
        // Scene order: mjv_updateScene's geoms first, then the markers
        // appended by add_marker, so markers land over the robot.
        for (int i = 0; i < scene_.ngeom; ++i)
        {
            ...
            case mjGEOM_MESH:
            {
                ...
                range = &mesh_ranges_[static_cast<size_t>(meshid)];
                break;
            }
            default:
