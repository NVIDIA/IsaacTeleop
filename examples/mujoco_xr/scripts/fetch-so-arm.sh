...
#
# Fetches the SO-101 leader-gripper assets this example draws, rather than
# vendoring 2.3 MB of binary STL that Git LFS made every clone pay for.
#
# Nothing calls this at build time, deliberately: an isolated PEP-517 wheel
# build must not reach the network, so this is an explicit step and the app
# fails at startup naming this script.
#
...
#
# The URDF is not decoration: it is where app.py's trigger hinge comes from (the
# `gripper` joint's origin and axis), and having it on disk is what lets
# test_ghost.py check those constants against their source.
ASSETS=(
