...
try
{
    ...
    isaaccapture::Logger::get("isaaccapture.plugins.so101_leader.main")
...
{
    isaaccapture::Logger::get("isaaccapture.plugins.so101_leader.main")->error("{}: {}", argv[0], e.what());
    return 1;
}
catch (...)
{
    isaaccapture::Logger::get("isaaccapture.plugins.so101_leader.main")->error("{}: Unknown error", argv[0]);
    return 1;
}
