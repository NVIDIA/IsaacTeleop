...
using namespace plugins::gamepad;

namespace
{

// Returns the first udev-classified joystick device (js API, not evdev), or
// nullopt if none found. Lets the plugin run with zero required arguments
// when launched via PluginManager (which invokes plugins as
// `<command> --plugin-root-id=<id>`, not positional args).
std::optional<std::string> discover_gamepad_device_path()
{
...
struct ParsedArgs
{
    ...
    std::string collection_id = "gamepad";
};
...
try
{
    if (argc == 0)
    {
        std::cerr << "Usage: gamepad_plugin [device_path] [--plugin-root-id=<collection_id>]" << std::endl;
        return 1;
    }
    ...
    if (!device_path)
        device_path = discover_gamepad_device_path();
    if (!device_path)
    {
        std::cerr << argv[0] << ": No joystick device found under /dev/input/by-path/ and none given explicitly."
                  << std::endl;
        return 1;
    }

    std::cout << "Gamepad (device: " << *device_path << ", collection: " << args.collection_id << ")" << std::endl;

    GamepadPlugin plugin(*device_path, args.collection_id);
