...
try
{
    auto logger = isaaccapture::Logger::get("isaaccapture.plugins.haptikos.main");
    ...
    logger->info("Haptikos Hands Plugin");
...
{
    isaaccapture::Logger::get("isaaccapture.plugins.haptikos.main")->error("{}: {}", argv[0], e.what());
    return 1;
}
catch (...)
{
    isaaccapture::Logger::get("isaaccapture.plugins.haptikos.main")->error("{}: Unknown error", argv[0]);
    return 1;
}
