...
#
# Entry point for SENSING SG8A SIPL camera setup. Detects whether it is running
# on the Jetson host or inside a container and delegates to the right half;
# every argument is forwarded unchanged.
#
# Setup is genuinely two-sided: installing the vendor drivers and selecting the
# device-tree overlay only happen on the host, while device-node access and the
# build SDKs only matter inside the container. Run it in both places.
#
# Usage:
