...
struct JetsonEncoder::Impl
{
    ...
    static bool capture_dq(struct v4l2_buffer* v4l2_buf, NvBuffer* buffer, NvBuffer* /*shared*/, void* arg)
    {
        ...
        if (buffer && buffer->planes[0].bytesused > 0)
        {
            ...
            self->ready.emplace_back(data, data + buffer->planes[0].bytesused);
        }
...
bool JetsonEncoder::submit(uintptr_t rgba_device_ptr, std::size_t row_pitch_bytes)
{
    auto* enc = m_impl->encoder;
...
std::vector<uint8_t> JetsonEncoder::poll()
{
...
std::vector<uint8_t> JetsonEncoder::end_of_stream()
{
    auto* enc = m_impl->encoder;
