...
void ArgusCamera::initialize()
{
    ...
    // Orin (JetPack 6.2) rejects a display-agnostic EGLStream: the handle that
    // reaches cuEGLStreamConsumerConnect is unusable and CUDA reports
    // CUDA_ERROR_INVALID_HANDLE. Bind a real display when one is available and
    // fall back to display-agnostic for the headless Thor path below.
    ...
    // With DISPLAY set, GLVND hands out Mesa's EGL: Tegra's EGL cannot drive an
    // X11 display, so the Mesa ICD wins the vendor probe. Nothing NVIDIA-
    // specific works on that display -- Argus hands back EGL_NO_STREAM_KHR and
    // NvBufSurfaceMapEglImage fails with "Failed to create EGLImage" -- and both
    // read as Argus faults rather than as the wrong EGL library. Checking the
    // vendor turns that into one legible error. main() clears DISPLAY for this
    // reason; libnvbufsurface calls eglGetDisplay(EGL_DEFAULT_DISPLAY) itself,
    // so it cannot be steered from here with a display we picked.
    if (egl_initialized_)
    {
        ...
        {
            std::ostringstream oss;
            oss << "EGL vendor is '" << (vendor ? vendor : "unknown")
                << "', not NVIDIA. Argus and NvBufSurface need the Tegra EGL driver. "
                   "Unset DISPLAY before starting this plugin.";
            throw std::runtime_error(oss.str());
        }
    }
    ...
    for (size_t i = 0; i < stream_count_; ++i)
    {
        ...
        if (egl_initialized_)
        {
            check_argus(i_egl_settings->setEGLDisplay(egl_display_), "setEGLDisplay");
        }
...
void ArgusCamera::connect_cuda_consumers()
{
    for (size_t i = 0; i < stream_count_; ++i)
    {
        ...
        if (!streams_[i].connection)
        {
            ...
            // Multi-process Argus (libnvargus_socketclient) returns
            // EGL_NO_STREAM_KHR here on L4T R36.4.3: the stream lives in
            // nvargus-daemon, so a local CUDA consumer cannot attach and this
            // call fails with CUDA_ERROR_INVALID_HANDLE.
            EGLStreamKHR handle = streams_[i].egl_stream->getEGLStream();
            if (handle == EGL_NO_STREAM_KHR)
            {
                throw std::runtime_error(
                    "Argus returned EGL_NO_STREAM_KHR: multi-process Argus does not expose its EGLStream to a "
                    "local CUDA consumer on this L4T release. STREAM_TYPE_BUFFER + NvBufSurface is the "
                    "supported Argus-to-CUDA path here.");
            }
            check_cuda(cuEGLStreamConsumerConnect(&streams_[i].connection, handle), label.str().c_str());
        }
    }
}
