...
if [[ "${#MISSING_GIDS[@]}" -gt 0 ]]; then
    ...
    if in_container; then
        # Both halves are required. --group-add reaches PID 1 only; `docker exec`
        # resolves supplementary groups from the container's own /etc/group.
        hint "1. devcontainer.json runArgs:"
        for gid in "${MISSING_GIDS[@]}"; do hint "     \"--group-add\", \"$gid\","; done
        hint "2. Dockerfile — the same gids must exist in the image, or exec sessions"
        hint "   still will not see them (--group-add alone is NOT enough):"
        hint "     RUN groupadd -g <gid> <name> && usermod -aG <name> \$USERNAME"
        hint "Then REBUILD the container; a restart will not re-apply runArgs."
    else
...
if [[ -f /boot/tegra234-camera-sipl-camera-overlay.dtbo ]]; then
    ...
    hint "setup_host.sh --smoke-test"
else
    if in_container; then
        info "/boot not visible from here"
    else
        bad "overlay not installed — setup_host.sh --install-drivers"; note_fail
    fi
fi
...
else bad "jetson_multimedia_api not found — setup_container.sh fetches it"; note_fail; fi
if sipl="$(find_sipl_api)"; then ok "sipl api: $sipl"
else bad "jetson_sipl_api not found — setup_container.sh fetches it"; note_fail; fi
