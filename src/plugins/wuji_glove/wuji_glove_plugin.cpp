...
// src/python/isaaccapture/retargeters/wuji_hand_retargeter.py — same 21 entries, applied in the
...
XrPosef pose_from_env(const char* name, const XrPosef& fallback)
{
    ...
    auto logger = isaaccapture::Logger::get("isaaccapture.plugins.wuji_glove.main");
    XrPosef pose{};
...
plugin_utils::WristSourceMode wrist_source_mode_from_env()
{
    ...
    isaaccapture::Logger::get("isaaccapture.plugins.wuji_glove.main")
...
WujiGlovePlugin::WujiGlovePlugin(const std::string& plugin_root_id) noexcept(false) : m_root_id(plugin_root_id)
{
    ...
    m_logger->info("WujiGlovePlugin initialized and running");
}
