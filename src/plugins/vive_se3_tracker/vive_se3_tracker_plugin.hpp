...
// line) so readers (e.g. record_se3_vive.py) can auto-discover them. Placed under
// $XDG_RUNTIME_DIR (per-user, not world-writable) when set, else /tmp. Overridable
// wholesale via VIVE_SE3_COLLECTIONS_FILE. Rewritten on each new collection,
// removed on exit. Keep the resolution in sync with record_se3_vive.py.
constexpr const char* kCollectionsFileName = "vive_se3_collections.txt";
constexpr const char* kFallbackTmpDir = "/tmp";
