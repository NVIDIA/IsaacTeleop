...
- **Keep `Serialized<T>` schema-agnostic.** It owns a buffer and re-points within it; it knows nothing about any field. Anything that assumes a wrapper's `data` field belongs in `<schema/tracked.hpp>` — `narrow_payload(handle)` — so a translation unit's includes say which it depends on.
