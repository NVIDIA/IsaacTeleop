...
def resolve_tracker_entry(
    ...
    direction = str(
        raw.get("direction", defaults_doc.get("defaults", {}).get("direction", "in"))
    )
    ...
    if resolved["direction"] == "in" and record:
        for key in ("mcap_channels", "replay_channels"):
            if key not in resolved:
                raise ValueError(
                    f"tracker '{raw['name']}': missing required key '{key}' for direction=in"
                )
