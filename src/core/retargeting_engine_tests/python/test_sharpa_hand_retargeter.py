...
Uses a stripped-down MJCF (no mesh references) bundled in test_assets/.
Requires the ``retargeters`` optional dependencies (pinocchio, pink, …).
"""
...
SHARPA_MJCF = str(TEST_ASSETS_DIR / "right_sharpawave_nomesh.xml")
...
class TestSharpaHandRetargeter:
    ...
    def test_warm_starting_persistence(self, retargeter):
        ...
        # but not identical (different input)
        finger_start = retargeter._finger_qpos_start
        finger_end = retargeter._finger_qpos_end
        diff = np.linalg.norm(
            qpos_after_second[finger_start:finger_end]
            - qpos_after_first[finger_start:finger_end]
        )
        assert diff < 1.0, (
