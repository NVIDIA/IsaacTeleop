...
class TestParameterState:
    ...
    def test_register_parameter(self):
        ...
        float_param = FloatParameter(name="alpha", description="Alpha", min_value=0.0, max_value=1.0)
    ...
    def test_reset_to_defaults(self):
        ...
        float_param = FloatParameter(name="alpha", description="Alpha", default_value=0.5)
    ...
    def test_save_and_load_to_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            state = ParameterState(name="test", parameters=parameters, config_file=config_path)
            ...
            state2 = ParameterState(name="test", parameters=parameters2, config_file=config_path)
            ...
            np.testing.assert_array_almost_equal(values["position"], np.array([4.0, 5.0, 6.0]))
    
    def test_load_nonexistent_file_doesnt_error(self):
        """Test loading from nonexistent file doesn't error."""
        param = FloatParameter(name="value", description="Value", default_value=1.0)
        state = ParameterState(name="test", parameters=[param], config_file="/nonexistent/path.json")
    ...
    def test_load_from_file_after_save(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            state = ParameterState(name="test", parameters=[param], config_file=config_path)
    ...
    def test_config_path_property(self):
        ...
        param = FloatParameter(name="value", description="Value", default_value=1.0)
        state = ParameterState(name="test", parameters=[param], config_file="/path/to/config.json")
