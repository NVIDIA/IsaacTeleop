...
"""
End-to-end tests for the gamepad device: raw joystick-API button/axis state
(constructed via the real schema Python bindings, no live device/plugin/OpenXR session
involved) flowing through GamepadSource into GamepadToSe3RelRetargeter,
GamepadGripperRetargeter, and GamepadToSe2Retargeter.
...
def _run_source(
    ...
    if pressed_buttons is None:
        tracked = GamepadOutputTrackedT()  # data is None -> inactive
    else:
        tracked = GamepadOutputTrackedT(
            GamepadOutput(pressed_buttons, axes or [0.0] * 8, True)
        )
...
class TestGamepadEndToEnd:
    ...
    def test_se3_dead_zone_suppresses_small_deflection(self):
    ...
    def test_inactive_device_yields_safe_defaults(self):
        ...
        assert src_outputs["gamepad_axes"].is_none
        assert src_outputs["gamepad_buttons"].is_none
    ...
    def test_buttons_bitmap_covers_indices_outside_gripper_button(self):
        ...
        bitmap = np.asarray(src_outputs["gamepad_buttons"][0])
        assert bitmap[5] == 1
        assert bitmap.sum() == 1
    ...
    def test_se2_left_and_right_stick_combine(self):
    ...
    def test_se2_dead_zone_suppresses_small_deflection(self):
        """A deflection smaller than the configured dead zone is treated as zero."""
        src = _gamepad_source()
        axes = _axes({AXIS_LEFT_Y: -0.005})
        src_outputs = _run_source(src, [], axes=axes)
