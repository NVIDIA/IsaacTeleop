...
class TestSimpleConnection:
    ...
    def test_chain_three_retargeters(self):
        ...
        connected_double = double.connect({
            "x": source.output("value")
        })
        
        connected_triple = triple.connect({
            "x": connected_double.output("result")
        })
...
class TestMultipleInputConnections:
    ...
    def test_connect_add_with_two_sources(self):
        ...
        outputs = connected({
            "source1": {"dummy": dummy_input1},
            "source2": {"dummy": dummy_input2}
        })
...
class TestMultipleOutputConnections:
    ...
    def test_connect_to_specific_output(self):
        ...
        connected_multi = multi.connect({
            "x": source.output("value")
        })
        
        # Connect double to multi's "doubled" output
        connected_double = double.connect({
            "x": connected_multi.output("doubled")
        })
    ...
    def test_connect_to_different_outputs(self):
        ...
        connected_triple = triple.connect({
            "x": connected_multi.output("tripled")
        })
...
class TestComplexGraphs:
    ...
    def test_deep_chain(self):
        ...
        for i in range(4):
            ...
            if i == 0:
                connected = double.connect({
                    "x": retargeters[-1].output("value")
                })
            else:
                connected = double.connect({
                    "x": retargeters[-1].output("result")
                })
...
class TestExecutionCaching:
    ...
    def test_diamond_graph_caching(self):
        ...
        connected_counter = counter.connect({
            "x": source.output("value")
        })
        connected_left = left.connect({
            "x": connected_counter.output("result")
        })
        connected_right = right.connect({
            "x": connected_counter.output("result")
        })
        connected_add = add.connect({
    ...
    def test_multi_output_caching(self):
        ...
        connected_left = left.connect({
            "x": connected_multi.output("result")
        })
        connected_right = right.connect({
            "x": connected_multi.output("result")
        })
    ...
    def test_complex_dag_caching(self):
        ...
        # Build the graph
        connected_count1 = count1.connect({
            "x": source.output("value")
        })
        connected_count2 = count2.connect({
            "x": source.output("value")
        })
        ...
        connected_left = left.connect({
            "x": connected_add_temp.output("sum")
        })
        
        # Right also uses count2
        connected_right = right.connect({
            "x": connected_count2.output("result")
        })
    ...
    def test_subgraph_caching(self):
        ...
        add = AddRetargeter("add")
        connected_add = add.connect({
            "a": connected.output("result"),
            "b": connected.output("result")
        })
...
class TestOutputCombiner:
    ...
    def test_combine_two_outputs(self):
        ...
        combiner = OutputCombiner({
            "first": source1.output("value"),
            "second": source2.output("value")
        })
        ...
        outputs = combiner({
            "source1": {"dummy": dummy_input},
            "source2": {"dummy": dummy_input}
        })
    ...
    def test_combiner_as_source(self):
        ...
        add = AddRetargeter("add")
        connected_add = add.connect({
            "a": combiner.output("doubled"),
            "b": combiner.output("tripled")
        })
    ...
    def test_combiner_caching(self):
        ...
        add = AddRetargeter("add")
        connected_add = add.connect({
            "a": combiner.output("result1"),
            "b": combiner.output("result2")
        })
