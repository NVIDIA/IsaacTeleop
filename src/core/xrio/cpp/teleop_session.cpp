...
std::shared_ptr<TeleopSession> TeleopSession::Create(
    const std::vector<std::shared_ptr<ITracker>>& trackers,
    const OpenXRSessionHandles& handles) {
    
    auto session = std::shared_ptr<TeleopSession>(
        new TeleopSession(handles));
...
TeleopSession::TeleopSession(const OpenXRSessionHandles& handles)
    : handles_(handles),
...
std::shared_ptr<TeleopSession> TeleopSessionBuilder::build(
    const OpenXRSessionHandles& handles) {
