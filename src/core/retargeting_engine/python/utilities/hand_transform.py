...
class HandTransform(BaseRetargeter):
    ...
    def _transform_hand(
        ...
        # Transform all joint positions (26, 3)
        positions = np.from_dlpack(inp[HandInputIndex.JOINT_POSITIONS])
        out[HandInputIndex.JOINT_POSITIONS] = transform_positions_batch(
            positions, rotation, translation
        )

        # Transform all joint orientations (26, 4)
        orientations = np.from_dlpack(inp[HandInputIndex.JOINT_ORIENTATIONS])
        out[HandInputIndex.JOINT_ORIENTATIONS] = transform_orientations_batch(
            orientations, rotation
        )

        # Pass through unchanged fields
        out[HandInputIndex.JOINT_RADII] = inp[HandInputIndex.JOINT_RADII]
        out[HandInputIndex.JOINT_VALID] = inp[HandInputIndex.JOINT_VALID]
        out[HandInputIndex.IS_ACTIVE] = inp[HandInputIndex.IS_ACTIVE]
        out[HandInputIndex.TIMESTAMP] = inp[HandInputIndex.TIMESTAMP]
