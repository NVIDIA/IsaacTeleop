...
class HandsSource(BaseRetargeter):
    ...
    def _update_hand_data(self, group: TensorGroup, hand_data: Any) -> None:
        ...
        # Extract joint data
        if hand_data.joints is not None:
            for i in range(len(hand_data.joints)):
                joint = hand_data.joints[i]
                pos = joint.pose.position
                positions[i] = [pos.x, pos.y, pos.z]
                ori = joint.pose.orientation
                orientations[i] = [ori.w, ori.x, ori.y, ori.z]
