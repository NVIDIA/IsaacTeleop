...
class GamepadSource(IDeviceIOSource):
    ...
    def poll_tracker(self, deviceio_session: Any) -> RetargeterIO:
        ...
        Returns:
            Dict with "deviceio_gamepad" TensorGroup containing GamepadOutputTrackedT.
        """
        tracked = self._gamepad_tracker.get_gamepad_data(deviceio_session)
    ...
    def _compute_fn(self, inputs: RetargeterIO, outputs: RetargeterIO, context) -> None:
        """
        Convert DeviceIO GamepadOutputTrackedT to the standard gamepad outputs.

        Calls ``set_none()`` on both outputs when the gamepad plugin has not yet
        streamed.

        Args:
            inputs: Dict with "deviceio_gamepad" containing GamepadOutputTrackedT wrapper
        ...
        tracked: GamepadOutputTrackedT = inputs["deviceio_gamepad"][0]
        state: GamepadOutput | None = tracked.data
