...
"""
Passthrough Input Node - Generic typed placeholder for external graph inputs.
...
class PassthroughInput(BaseRetargeter):
    """
    Generic passthrough node representing a single external input in the graph.
    ...
    PassthroughInput for the externally-provided ones so they can participate
    in connect() wiring.
    ...
    Example:
        ...
        # At runtime, TeleopSession feeds data to the passthrough node:
    ...
    def __init__(self, name: str, tensor_type: TensorGroupType) -> None:
        """
        Initialize a passthrough input node.
