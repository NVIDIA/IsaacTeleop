...
# Global flag to enable/disable runtime validation (for performance)
_RUNTIME_VALIDATION_ENABLED = os.environ.get("RETARGETING_RUNTIME_VALIDATION", "1") == "1"
...
def is_runtime_validation_enabled() -> bool:
    ...
    return _RUNTIME_VALIDATION_ENABLED
...
class Tensor:
    ...
    def __init__(self, tensor_type: 'TensorType', value: Any = None) -> None:
        ...
        # If value provided (not None), validate and set it
        if value is not None:
            if not tensor_type.validate_value(value):
                error_msg = tensor_type.get_validation_error(value)
                raise TypeError(
                    f"Invalid value for tensor '{tensor_type.name}': {error_msg}"
                )
    ...
    def value(self) -> Any:
        ...
        if _RUNTIME_VALIDATION_ENABLED:
            if not self._type.validate_value(self._value):
                error_msg = self._type.get_validation_error(self._value)
                raise RuntimeError(
                    f"Tensor value corrupted for '{self._type.name}': {error_msg}"
                )
        return self._value
    ...
    def value(self, new_value: Any) -> None:
        ...
        """
        if not self._type.validate_value(new_value):
            error_msg = self._type.get_validation_error(new_value)
            raise TypeError(
                f"Invalid value for tensor '{self._type.name}': {error_msg}"
            )
