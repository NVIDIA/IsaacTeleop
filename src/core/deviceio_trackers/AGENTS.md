...
- **`__init__.py` needs no edit for new manifest trackers.**
  [`../../python/isaaccapture/deviceio_trackers/__init__.py`](../../python/isaaccapture/deviceio_trackers/__init__.py)
  ...
  because it is not authored under ``src/python/``) and splices its ``__all__``. The legacy `isaaccapture.deviceio` shim is a **frozen** compat
