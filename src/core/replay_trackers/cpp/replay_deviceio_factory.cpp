...
std::unique_ptr<ITrackerImpl> try_create_steering_wheel_impl(ReplayDeviceIOFactory& factory, const ITracker& tracker)
{
...
inline const TryCreateFn k_tracker_dispatch[] = {
    ...
    &try_create_steering_wheel_impl,
...
std::unique_ptr<ISteeringWheelTrackerImpl> ReplayDeviceIOFactory::create_steering_wheel_tracker_impl(
    const SteeringWheelTracker* tracker)
{
    return std::make_unique<ReplaySteeringWheelTrackerImpl>(open_reader(filename_), get_name(tracker));
}
