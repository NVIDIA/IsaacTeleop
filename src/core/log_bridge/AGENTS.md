...
`ISAACTELEOP_LOG_SOCKET` selects `SocketForwardSink`, including loopback from
...
`ISAACTELEOP_LOG_LEVEL` (info by default), while the file always captures
trace and above.
...
`sink_config.cpp` and `socket_sink.cpp` read `ISAACTELEOP_LOGGING`, `ISAACTELEOP_LOG_DIR`,
