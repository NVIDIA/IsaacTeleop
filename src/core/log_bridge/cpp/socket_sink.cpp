...
bool SocketForwardSink::ensure_connected()
{
    ...
    // Bounds ::send() below, which otherwise blocks indefinitely once the
    // receiver's socket buffer fills -- and base_sink holds this sink's mutex
    // across sink_it_(), so every other logging thread would pile up behind it.
    // A record dropped on timeout is the best-effort contract; a hung tracking
    // loop is not.
