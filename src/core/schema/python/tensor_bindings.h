...
inline py::array tensor_to_numpy(const TensorT& tensor) {
    ...
    py::array result(py::buffer_info(
        nullptr,  // Pointer to buffer (nullptr for allocation).
        itemsize,
        format,
        static_cast<py::ssize_t>(tensor.ndim),
        shape,
        strides
    ));
...
inline void bind_tensor(py::module& m) {
    ...
    py::class_<DLDataType>(m, "DLDataType")
        .def(py::init<>())
        .def(py::init<DLDataTypeCode, uint8_t, uint16_t>(),
             py::arg("code"), py::arg("bits"), py::arg("lanes") = 1)
    ...
    py::class_<DLDevice>(m, "DLDevice")
        ...
        .def(py::init<DLDeviceType, int32_t>(),
             py::arg("device_type"), py::arg("device_id") = 0)
    ...
    py::class_<TensorT, std::unique_ptr<TensorT>>(m, "TensorT")
        ...
        .def_static("from_numpy", &numpy_to_tensor,
                    py::arg("array"),
                    "Create a TensorT from a numpy array.")
        .def("to_numpy", [](const TensorT& self) { return tensor_to_numpy(self); },
