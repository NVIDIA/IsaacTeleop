...
@pytest.mark.parametrize(
    "initial_cfg,config_payload",
    [
        ({"serverIP": "127.0.0.1", "port": 49100}, {}),
        (
            {"serverIP": "10.0.0.1", "port": 9000},
            {"serverIP": "10.0.0.1", "port": 9000},
        ),
    ],
)
async def test_set_config_noop_when_unchanged(
    initial_cfg: dict, config_payload: dict
) -> None:
    hub = OOBControlHub(initial_config=initial_cfg)
    hw = QueueWS()
    dw = QueueWS()
    th = asyncio.create_task(hub.handle_connection(hw))
    td = asyncio.create_task(hub.handle_connection(dw))
    ...
    await dw.inject(
        json.dumps({"type": "setConfig", "payload": {"config": config_payload}})
    )
    await asyncio.sleep(0)
...
@pytest.mark.asyncio
async def test_dashboard_command_message_unknown() -> None:
    hub = OOBControlHub()
    hw = QueueWS()
    dw = QueueWS()
    th = asyncio.create_task(hub.handle_connection(hw))
    ...
    assert "command" in errs[0]["payload"]["message"]
...
async def test_dashboard_set_config_requires_token_when_configured() -> None:
    hub = OOBControlHub(control_token="t")
    hw = QueueWS()
    dw = QueueWS()
    th = asyncio.create_task(hub.handle_connection(hw))
    td = asyncio.create_task(hub.handle_connection(dw))
...
@pytest.mark.asyncio
async def test_http_oob_set_config_merges_client_ui_fields() -> None:
    """Hub shallow-merge keeps arbitrary client UI keys for the WebXR allowlist."""
    hub = OOBControlHub(initial_config={"serverIP": "127.0.0.1", "port": 49100})
    hw = QueueWS()
    th = asyncio.create_task(hub.handle_connection(hw))
    await hw.inject(json.dumps({"type": "register", "payload": {"role": "headset"}}))
    ...
    assert body.get("changed") is True
    snap = await hub.get_snapshot()
    assert snap["config"]["codec"] == "h265"
    assert snap["config"]["panelHiddenAtStart"] is True
    assert snap["config"]["perEyeWidth"] == 1920
    cfg_msgs = [m for m in _loads_sent(hw) if m["type"] == "config"]
    assert cfg_msgs[0]["payload"]["config"]["codec"] == "h265"
