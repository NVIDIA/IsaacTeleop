...
int main(int argc, char* argv[])
{
    if (argc < 2)
    {
        std::cerr << "Usage: clock_client_xr <server_ip> [port] [interval_ms] [count]\n"
    ...
    std::shared_ptr<core::XrClockTranslator> xr_translator;
    try
    {
        xr_translator = core::XrClockTranslator::Create();
    }
    catch (const std::exception& e)
    {
        std::cerr << "[ClockClientXR] Failed to create XR translator: " << e.what() << std::endl;
        return 1;
    }
    ...
    try
    {
        client = core::ClockClient::Create(server_ip, port, on_measurement, xr_translator->make_translator());
    }
    catch (const std::exception& e)
    {
    ...
    std::cout << "[ClockClientXR] Pinging " << server_ip << ":" << port << " every " << interval_ms << " ms"
              << " (client: XrTime, server: CLOCK_MONOTONIC_RAW)\n\n";
