...
def _cache_dir(env_var: str, name: str) -> Path:
    ...
    if not override and os.environ.get(legacy_var, "").strip():
        # Warned about, not read: unread, it silently swaps curated site assets for
        # upstream geometry. TODO(1.9): drop with the isaacteleop distribution.
        LOG.warning(
...
def _fetch_failure(dest: Path, env_var: str, cause: OSError) -> OSError:
    ...
    # Offered only for the default location: an override names curated site assets,
    # which a 1.5 copy must not be poured into. migration.rst's recipe is `cp -a` with
    # a trailing `/.`, not `mv` -- dest exists, so mv nests the cache in it and exits 0.
...
def ensure_rebot_devarm_rs_scene() -> Path:
    ...
    if not marker.exists() and _rebot_cached_digest(dest) == REBOT_MANIFEST_SHA256:
        # A cache pre-populated through REBOT_CACHE_ENV_VAR arrives without the
