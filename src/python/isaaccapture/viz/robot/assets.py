...
def _cache_dir(env_var: str, name: str) -> Path:
    ...
    legacy_var = env_var.replace("ISAACCAPTURE_", "ISAACTELEOP_", 1)
    if not override and os.environ.get(legacy_var, "").strip():
        # Warn about it rather than read it. An operator with curated site assets
        # under the old variable is on the path where nothing else says anything:
        # the override goes unread, the fetch succeeds, and they run upstream
        # geometry instead of their patched meshes.
        # TODO(1.9): drop with the isaacteleop compatibility distribution.
        LOG.warning(
            "Robot twin: %s is set but no longer read. Export %s instead.",
            legacy_var,
            env_var,
        )
    dest = Path(override) if override else _cache_root() / "isaaccapture" / name
    dest.mkdir(parents=True, exist_ok=True)
    return dest


def _fetch_failure(dest: Path, env_var: str, cause: OSError) -> OSError:
    """The download failed: name the levers this operator actually has."""
    parts = [f"robot twin: could not fetch assets into {dest} ({cause})."]
    legacy = _cache_root() / "isaacteleop" / dest.name
    # Claimed unconditionally, this is the lie the _cache_dir diagnostic exists to
    # avoid: on a fresh host there is no 1.5 cache. Only offered for the default
    # location: an override names curated site assets, which a 1.5 copy must not
    # be poured into.
    if not os.environ.get(env_var, "").strip() and legacy.is_dir():
        # migration.rst's recipe, and shell-quoted -- a cache path with a space
        # otherwise splits mid-component and the command operates on a neighbour.
        # Not `mv`: _cache_dir has already mkdir'd dest and _copy_wrappers has
        # already filled it, so mv puts legacy *inside* it and exits 0. The
        # trailing `/.` carries `.fetch_complete`, which is what gates the fetch.
        parts.append(
            f"A 1.5 cache is still on disk under the old name -- "
            f"`mkdir -p {shlex.quote(str(dest))} && "
            f"cp -a {shlex.quote(f'{legacy}/.')} {shlex.quote(f'{dest}/')}`."
        )
    parts.append(
        f"On a host with no route to GitHub, point {env_var} at a pre-populated "
        f"directory instead."
    )
    return OSError(" ".join(parts))
...
def ensure_so101_scene() -> Path:
    ...
    if not marker.exists():
        try:
            ...
            raise _fetch_failure(dest, CACHE_ENV_VAR, exc) from exc
        marker.touch()
    ...
    if not marker.exists():
        ...
        try:
            for name in _menagerie_rebot_files():
                # Everything lands flat: MuJoCo drops an included file's own meshdir,
                # so upstream's meshdir="assets" is inert once rebot_arm.xml includes it.
                flat = Path(name).name
                remote = (
                    f"{base}/assets/{flat}"
            ...
            raise _fetch_failure(dest, REBOT_CACHE_ENV_VAR, exc) from exc
