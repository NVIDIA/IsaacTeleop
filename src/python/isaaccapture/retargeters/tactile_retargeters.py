...
class TactileVectorToFingerPower(BaseRetargeter):
    ...
    Inputs:
        - ``"tactile"``: :func:`TactileVector(num_taxels) <isaaccapture.retargeting_engine.tensor_types.TactileVector>`
          -- typically each taxel is the contact-force magnitude on one finger
          pad, but the mapping is configurable via ``finger_groups``.

    Outputs:
        - ``"powers"``: :func:`FingerPowerVector(num_fingers) <isaaccapture.retargeting_engine.tensor_types.FingerPowerVector>`
          in ``[0, 1]``.
...
class TactileHeatmapToFingerPower(BaseRetargeter):
    ...
    Inputs:
        - ``"heatmap"``: :func:`TactileHeatmap(rows, cols, num_pads) <isaaccapture.retargeting_engine.tensor_types.TactileHeatmap>`
...
class TactileHeatmapToWristPulse(BaseRetargeter):
    ...
    Outputs:
        - ``"power"``: :func:`FingerPowerVector(1) <isaaccapture.retargeting_engine.tensor_types.FingerPowerVector>`
...
class TactileVectorToControllerPulse(BaseRetargeter):
    ...
    Outputs:
        - ``"pulse"``: :func:`ControllerHapticPulse <isaaccapture.retargeting_engine.tensor_types.ControllerHapticPulse>`
