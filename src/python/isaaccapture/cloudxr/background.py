...
#: Module names earlier releases spawned the detached service under.  Recognised,
#: never spawned: these services are detached by design and outlive the shell, so
#: an upgrade meets one whose /proc cmdline is a fact on disk that no import alias
#: can rewrite.  TODO(1.9): drop with the isaacteleop compatibility distribution.
...
def _is_our_service(pid: int) -> bool:
    ...
    The single choke point: ``read_pid`` gates ``terminate`` and ``read_run_flags``
    on it, so failing to recognise a service makes ``stop`` unlink the pid file and
    report "No detached CloudXR service is running" with exit 0 while that service
    still holds the GPU.
    """
    try:
