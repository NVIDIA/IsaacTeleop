...
def _clamp_orientation_step(
    ...
    # Renormalize, because the result becomes the next frame's ``previous``.
    # The pass-through branch is prev (x) (prev^-1 (x) target), whose norm is
    # |prev|^2 -- fed back every frame that SQUARES the deviation from unit, so
    # float64's 1e-16 reaches float32 overflow in ~60 frames (0.9 s at 72 Hz)
    # and the pose goes non-finite. Do not drop this.
