...
#: enough for a float32 round-trip and renormalisation drift, far tighter than the
#: shrinkage :func:`~isaacteleop.viz.robot.quaternion.rotate` would otherwise apply in
#: silence.
UNIT_QUAT_TOL = 1e-3
...
def is_unit(q: np.ndarray) -> bool:
    ...
    Layout-agnostic -- a norm does not care whether it is handed wxyz or xyzw. Every
    function here that turns something rejects exactly what this rejects, so a caller
    reading a quaternion off a device gets one predicate to gate on rather than a
    tolerance to re-guess.
    """
...
def yaw_of_direction(forward_xr: np.ndarray, fallback_xr: np.ndarray) -> np.ndarray:
    ...
    and at vertical (past it, the bearing reverses). This is the single definition of
    bearing, so anything built on it tracks a world-vertical turn 1:1.
    """
...
def yaw_of_axis(q_xyzw: np.ndarray, forward_local: np.ndarray) -> np.ndarray:
    ...
    # quaternion does here if it is let through. Callers reading a quaternion off a
    # device gate on `is_unit` rather than catch this: it raises on the frame loop.
