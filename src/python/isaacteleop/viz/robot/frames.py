...
Re-exported rather than reimplemented so ``cpp/frames.hpp`` and ``cpp/glcamera.hpp``
stay the one definition of each: the renderer uses these very functions, so a caller
testing against them is testing what runs.

.. warning::
   :data:`TRANS_MJ_FROM_XR` is a **calibration compiled into the wheel**, not a
   convention. Its ``x`` is how far in front of the operator the scene's origin sits and
   its ``z`` a floor datum, both authored for one example's workspace. Only *static*
   scene content sees it -- content posed from a controller or drawn at the eye goes out
   through the same transform it came in by, so both terms cancel. Make it a
   :class:`~isaacteleop.viz.robot.SceneTwin` argument before a second consumer relies on
   it.
"""
