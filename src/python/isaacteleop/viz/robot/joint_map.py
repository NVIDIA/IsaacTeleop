...
class JointMap:
    ...
    A twin is posed with a bare array, so the ORDER is the contract and the names are
    what pins it. The backend builds this from the loaded scene; the caller asserts it
    against the order it was authored for with :meth:`require`. Without that assert a
    scene whose joints were reordered upstream poses the wrong ones and still looks
    like a robot.

    Empty is legal, and is what a rigid scene has -- a prop moved only by its base pose.
    """
    ...
    def __init__(
        ...
        # Aliasing is the failure this catches: two names on one address means the
        # second silently overwrites the first every time a snapshot is scattered.
