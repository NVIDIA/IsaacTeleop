...
# Poll xrGetSystem until a headset shows up, rather than viz's own default of failing
# fast -- that default is for CI, and this class only ever runs against a headset.
# Matches oxr::OpenXRSession, which waits indefinitely for the same reason.
WAIT_FOR_HEADSET = -1
...
class XrTwinSession:
    ...
    def __init__(
        ...
        *,
        ...
        ``system_wait_seconds`` overrides how long ``__enter__`` polls ``xrGetSystem``;
        pass 0 to fail fast, as a headless test would.
        """
        self._twin = twin
        self._near_z = near_z
        self._far_z = far_z
    ...
    def __exit__(self, exc_type, exc_val, exc_tb) -> None:
        ...
        context before failing does not leak it. The frustum latch resets, so a reused
        instance re-checks on its next first frame.
        """
        try:
            self._checked_frustum = False
