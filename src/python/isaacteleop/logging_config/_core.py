...
"""Names, line format and levels shared by every handler here."""
...
def env_console_level() -> int:
    ...
    The six names above, case-insensitively, or a stdlib level number, which is
    taken literally. Anything else falls back to ``INFO`` rather than raising --
    ``install()`` runs from ``import isaacteleop``.
    """
