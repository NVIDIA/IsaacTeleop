...
The file handler always captures ``TRACE`` and above regardless of that.
...
separately, and without taking the host process's descriptors away from it.
``TeleopSession`` already wraps the calls that produce them, so an application
calls nothing for it; see ``_native_api.py`` and ``_native_fd.py`` for the
mechanism.
"""
...
# Four knobs and nothing else. The line format, the TRACE constant, the log
# directory and the native-capture helpers are reached through their own
# modules by the code in this tree that needs them; re-exporting one here makes
# it an interface this package has to keep. Raising one back up costs nothing
# once a host actually needs it -- see AGENTS.md.
__all__ = [
    "set_console_filter",
    "set_console_level",
    "set_logger_colors",
