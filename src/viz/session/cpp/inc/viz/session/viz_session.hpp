...
public:
    struct Config
    {
        ...
        // kXr-only: where the session's origin sits. kLocal (the default,
        // and the previous hard-wired behaviour) puts y=0 at HEAD HEIGHT.
        // An app that places world-locked geometry at a height above the
        // FLOOR must say kLocalFloor, or everything it draws lands about a
        // person too high. Unavailable spaces throw at session creation
        // naming the space, rather than silently degrading to a different
        // origin — a wrong origin is invisible in code and obvious only on
        // a headset.
