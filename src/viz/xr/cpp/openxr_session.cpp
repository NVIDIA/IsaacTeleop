...
{
    // WHAT THE RUNTIME OFFERS, logged beside what we asked for. There is no
    // fallback here on purpose: check_xr throws and names the space, which is
    // a loud failure the operator can act on, where quietly substituting a
    // different origin would render every world-locked thing at the wrong
    // height with nothing in the log. This line is what turns that throw from
    // "LOCAL_FLOOR failed" into "LOCAL_FLOOR failed, and here is what you can
    // have instead" — and it is the evidence needed before anyone adds a
    // fallback chain.
